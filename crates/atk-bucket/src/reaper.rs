//! TTL-based auto-cleanup and startup crash reconciliation for job sandboxes. Deliberately
//! OS-agnostic: it only talks to the `job_sandboxes` DB table and calls the shared
//! `remove_sandbox`, leaving "how do I actually force-kill this" to the OS-specific backend.
...
pub async fn reconcile_on_startup(pool: &SqlitePool, buckets_root: &Path) {
    ...
    for row in rows {
        ...
        if let Err(e) = super::remove_sandbox(pool, &handle).await {
