...
pub struct WorkflowRun {
    ...
    /// The last error from reporting this run's status back to GitHub (report_pending,
    /// report_success/failure), if any. `None` doesn't necessarily mean reporting succeeded, it
    /// may also mean there was no commit_sha to report against at all (e.g. a manual "Run now").
